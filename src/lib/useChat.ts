...
import { OpenRouterAdapter } from "@/infra/models/adapters/openrouter";
import { MegaLLMAdapter } from "@/infra/models/adapters/megallm";
import { AgentRouterAdapter } from "@/infra/models/adapters/agentrouter";
import { RoutewayAdapter } from "@/infra/models/adapters/routeway";
import { ModelRequest } from "@/infra/models/index";
...
export const useChat = () => {
  ...
  const sendMessage = useCallback(async (
    ...
    try {
      ...
      const aiMessageId = `msg-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
